...
interface UsernameInputProps extends CommonInputProps {}

interface ProfileInputFieldProps extends CommonInputProps {
  type?: string;
  multiline?: boolean;
}

interface SocialLinkInputProps extends CommonInputProps {
  // Add label prop for SocialLinkInput
  label: string;
}

interface GenericInputProps extends CommonInputProps {
  type: string; // Or you can use 'string' if the field can have any type
  multiline?: boolean;
}

const GenericInput: React.FC<CommonInputProps & GenericInputProps> = (
  props
) => {
  ...
  const getInputComponent = () => {
    switch (type) {
      case "text":
        return (
          <TextInput {...(rest as TextInputProps)} multiline={multiline} />
        );
      case "username":
        return <UsernameInput {...(rest as UsernameInputProps)} />;
      case "profile":
        return (
          <ProfileInputField
            {...(rest as ProfileInputFieldProps)}
            multiline={multiline}
          />
        );
      case "social":
        return <SocialLinkInput {...(rest as SocialLinkInputProps)} />;
      default:
        return null;
    }
  };
